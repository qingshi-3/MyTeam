...
- Each persistent roster hero owns its own automatic skill and Battle-local mana. Starting-hero identity never selects another hero's skill or grants a shared hero-mana budget. The existing independent tactical-command product is separate; it is not the hero skill interface.
...
- Ordinary attack delivery is explicitly authored as Melee, Projectile, or Beam. Melee and Beam resolve immediately; Beam also displays a short connecting effect. Projectile launches a straight-moving entity without launch-time damage and resolves only when its swept path collides with a living enemy. It does not home or hit allies; bodies intercept by contact order, blocked terrain stops it, and lifetime bounds misses. A dead intended target does not redirect the shot; a dead source does not cancel an in-flight shot while battle remains active. Terminal battle completion discards remaining shots. Existing piercing projectiles hit at most two distinct enemies along the flight path, with the secondary hit retaining 35% raw damage; ordinary attack mana is earned once at launch, not per collision.
...
- Successive 0.1-second simulation samples form one ordered continuous travel path. Presentation advances linearly along that path at the matching displayed-speed cadence, may consume several already queued segments in one rendered frame to prevent x4 backlog, and never restarts easing, lift, or movement state at each sample. Terrain corners remain ordered path vertices even when a low rendering rate cannot display every vertex as a separate frame.
- Movement presentation keeps catch-up lag bounded to roughly 0.25 seconds of effective presentation time at the supported rendering cadence after movement input stops. A fresh move created during a hitch cannot spend time from before that event or finish before any frame shows travel; when rendering resumes, visible continuity takes priority over impossible wall-clock catch-up during frames that were never drawn. Pause freezes in-progress interpolation exactly where it is, Lab single-step visibly synchronizes to the stepped authoritative position, and speed changes retime from current progress without restart, reversal, or snap.
- Movement may lift only the character art by roughly three pixels while travelling, returning to zero when travel settles. Health, hero/role markers, pointer selection origin, and the unit root remain on the projected movement segment. Repeated position samples preserve a bounded presentation-only move phase where the authored clip permits it; action one-shots may still override the move clip without stopping root travel.
