...
- A Lab configuration consists of uniquely identified player-hero and published PvE instances, exact current-baseline `10×6` physical cells, per-player-hero equipment instances, player-team Relic stacks, placement mode, explicit population input, and one fixed integer seed. Shared authored definitions remain read-only. Legacy `PrimaryHeroInstanceId`/HeroRule compatibility is an implementation boundary documented under System Design, not a player-facing rule that grants one hero automatic team ownership. This documentation alignment does not claim that legacy Lab behavior has been migrated or verified.
