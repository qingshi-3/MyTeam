...
public partial class UiSmoke : Node
{
    ...
    private async Task<int> RunAsync()
    {
        ...
        try
        {
            ...
            if (screens.GetChildCount() != 13 || !root.GetNode<Control>("Screens/MainMenuScreen").Visible ||
            ...
            if (heroTiles.Length != 8 || heroTiles.Any(tile => tile.GetNode<UnitPortrait>("%HeroPortrait").Definition is null))
                throw new InvalidOperationException("hero library did not author eight compact animated portrait tiles");
            ...
            VerifyHeroDetail(root, "hero_banner_marshal", "22", "0.2", "全军集结");
            VerifyReportAndSemanticResources();
            VerifyHeroDetail(root, "hero_bone_regent", "2", "75%", "80%");
            VerifyHeroDetail(root, "hero_brood_matriarch", "12%");
            VerifyHeroDetail(root, "hero_gear_architect", "攻击与移动等待");
            VerifyHeroDetail(root, "hero_crimson_count", "28%", "8%");
            VerifyHeroDetail(root, "hero_edge_ascetic", "35%");
            VerifyHeroDetail(root, "hero_hour_arbiter", "1.8", "1/2");
            VerifyHeroDetail(root, "hero_gilded_factor", "加急雇佣", "115%", "100%");
            var goldCost = root.GetNode<ResourceCostBadge>("Screens/HeroSelectScreen/Margin/Layout/Content/HeroDetailPanel/Layout/DetailScroll/Content/HeroAbilityPanel/Layout/Header/Costs/GoldCostBadge");
            if (!goldCost.Visible || goldCost.DisplayText != "5 金币")
                throw new InvalidOperationException("merchant command gold cost is not a structured detail badge");
            ...
            if (!overview.Visible || !summaryButton.Text.Contains("部署", StringComparison.Ordinal))
            ...
            var armyManaCost = armyUnitRow.GetNode<ResourceCostBadge>("%ManaCostBadge");
            if (!armyManaCost.Visible || armyManaCost.DisplayText != "1 MP")
                throw new InvalidOperationException("Army hero command cost is not a structured mana badge");
            ...
            if (battleRoute is not null)
            {
                ...
                if (cells.Length != 6 || cells.Any(cell => cell.Slot < 0))
                    throw new InvalidOperationException("deployment board did not author six interactive anchors");
                if (!overview.Visible || !root.GetNode<Button>("ArmyOverview/SummaryButton").Text.Contains("后备", StringComparison.Ordinal))
            ...
            GD.Print("UI_SMOKE_OK screens=13 navigation=menu-hero-tower-report-reward-recruitment-result-settings hero=command-details recruitment=localized-traits,single-column,fixed-actions,isolated-screen merchant=economy-hud hierarchy=structured-cards,semantic-theme,icons portraits=hero,recruitment,deployment,army,report report=authored-responsive,active-tabs,seconds-only,explicit-zero,theme-tints interaction=modal-focus,battle-selection,zero-mana");
            return 0;
        }
    ...
    private static void VerifyReportAndSemanticResources()
    {
        foreach (var path in new[]
                 {
                     ...
                     "res://scenes/ui/components/BattleReportMetric.tscn",
        ...
        var themeSource = FileAccess.GetFileAsString("res://content/ui/game_theme.tres");
        foreach (var variation in new[]
                 {
                     "HeroLabel", "PlayerLabel", "HealingLabel", "EnemyLabel", "WarningLabel", "SecondaryLabel",
                     "HealingTitleLabel", "EnemyTitleLabel", "WarningTitleLabel"
                 })
        ...
        ulong seed)
    {
        ...
        var app = new RunApplication(content, save,
        [
        ...
        SetPrivate(root, "_encounter", encounter);
        InvokePrivate(root, "ResetPendingBattleFlow");
        InvokePrivate(root, "Show", root.GetNode<Control>("Screens/BattleScreen"));
        InvokePrivate(root, "OnBattleFinished", result);
        InvokePrivate(root, "OnBattleFinished", result);
        ...
        InvokePrivate(root, "OnBattleEndTransitionFinished");
        InvokePrivate(root, "OnBattleEndTransitionFinished");
        var report = root.GetNode<BattleReportScreen>("Screens/BattleReportScreen");
        var metrics = root.GetNode<HBoxContainer>("Screens/BattleReportScreen/Margin/Panel/Layout/ReportMetrics");
        var playerTab = root.GetNode<Button>("Screens/BattleReportScreen/Margin/Panel/Layout/Tabs/PlayerTab");
        var enemyTab = root.GetNode<Button>("Screens/BattleReportScreen/Margin/Panel/Layout/Tabs/EnemyTab");
        var duration = root.GetNode<Label>("Screens/BattleReportScreen/Margin/Panel/Layout/Header/ReportDuration/Text").Text;
        var outcomeTitle = root.GetNode<Label>("Screens/BattleReportScreen/Margin/Panel/Layout/Header/Headline/ReportOutcome");
        if (!report.Visible || metrics.GetChildCount() != 5 ||
            root.GetNode<VBoxContainer>("Screens/BattleReportScreen/Margin/Panel/Layout/RosterScroll/ReportRows").GetChildCount() == 0)
            throw new InvalidOperationException("battle report did not bind authored metrics/rows: " + label);
        var reportRows = root.GetNode<VBoxContainer>("Screens/BattleReportScreen/Margin/Panel/Layout/RosterScroll/ReportRows")
            .GetChildren().OfType<BattleReportUnitRow>().ToArray();
        if (reportRows.Any(row => row.GetNode<UnitPortrait>("%UnitPortrait").Definition is null))
            throw new InvalidOperationException("battle report rows did not reuse authored portraits: " + label);
        ...
        foreach (var metric in metrics.GetChildren().OfType<BattleReportMetric>())
        {
            var valueLabel = metric.GetNode<Label>("%MetricValue");
            var icon = metric.GetNode<TextureRect>("%MetricIcon");
            if (!icon.Modulate.IsEqualApprox(valueLabel.GetThemeColor("font_color")))
                throw new InvalidOperationException("battle report metric icon tint diverged from its Theme variation: " + label);
        }
        var metricFacts = metrics.GetChildren().OfType<BattleReportMetric>()
            .ToDictionary(metric => metric.GetNode<Label>("%MetricLabel").Text, metric => metric.GetNode<Label>("%MetricValue").Text);
        if (metricFacts.Count != 5 || metricFacts.Values.Any(string.IsNullOrWhiteSpace))
            throw new InvalidOperationException("battle report rendered an empty summary metric: " + label);
        if (outcome == BattleOutcome.PlayerDefeat &&
            (!metricFacts.TryGetValue("有效伤害", out var damageValue) || damageValue != "0"))
            throw new InvalidOperationException("defeat report did not render zero effective damage explicitly");
        Press(root, "Screens/BattleReportScreen/Margin/Panel/Layout/Tabs/EnemyTab");
        if (root.GetNode<VBoxContainer>("Screens/BattleReportScreen/Margin/Panel/Layout/RosterScroll/ReportRows").GetChildCount() == 0 ||
            playerTab.ButtonPressed || !enemyTab.ButtonPressed || playerTab.Disabled || enemyTab.Disabled)
    ...
    private static BattleResult SyntheticResult(BattleConfig config, BattleOutcome outcome)
    {
        ...
        return new BattleResult(outcome, 25, new string('a', 64), units, 3);
    }
    ...
    private static void VerifyRecruitmentCard(GameRoot root)
    {
        var application = (RunApplication?)typeof(GameRoot).GetField("_app", BindingFlags.Instance | BindingFlags.NonPublic)?.GetValue(root)
            ?? throw new InvalidOperationException("run application unavailable");
        var heroId = application.ActiveRun?.HeroId ?? throw new InvalidOperationException("active run unavailable");
        var found = false;
        ...
        InvokePrivate(root, "ShowRecruitment");
    ...
    {
        InvokePrivate(root, "ShowCombatReward");
        await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
    ...
    private async Task VerifyMerchantHudAsync(ContentRegistry content)
    {
        ...
        try
        {
            ...
            screen.StartBattle(content, MerchantConfig(hero, enemy, rule, summon, 5), "商人指令测试");
            await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
            var status = screen.GetNode<Label>("%BattleStatus");
            var command = screen.GetNode<Button>("Margin/Layout/Hud/HeroCommandHud/Layout/Mana/CommandButton");
            var commandName = screen.GetNode<Label>("Margin/Layout/Hud/HeroCommandHud/Layout/Info/CommandName");
            var manaCost = screen.GetNode<ResourceCostBadge>("Margin/Layout/Hud/HeroCommandHud/Layout/Mana/Costs/ManaCostBadge");
            var goldCost = screen.GetNode<ResourceCostBadge>("Margin/Layout/Hud/HeroCommandHud/Layout/Mana/Costs/GoldCostBadge");
            var mana = screen.GetNode<Label>("Margin/Layout/Hud/HeroCommandHud/Layout/Mana/ManaText");
            if (!commandName.Text.Contains("加急雇佣", StringComparison.Ordinal) || manaCost.DisplayText != "1 MP" || goldCost.DisplayText != "5 金币" ||
                !mana.Text.Contains("3/3", StringComparison.Ordinal) ||
                !status.Text.Contains("剩余金币 5", StringComparison.Ordinal))
                throw new InvalidOperationException("merchant command cost and balance are not visible");
            command.EmitSignal(BaseButton.SignalName.Pressed);
            if (screen.CurrentMana != 2 || screen.RemainingGold != 0 || screen.TemporaryUnitCount != 1 || !mana.Text.Contains("2/3", StringComparison.Ordinal) ||
                !status.Text.Contains("剩余金币 0", StringComparison.Ordinal))
            ...
            await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
            var selectedPanel = screen.GetNode<Control>("Margin/Layout/BattleBoard/SelectedUnitPanel");
            var selectedTitle = screen.GetNode<Label>("Margin/Layout/BattleBoard/SelectedUnitPanel/Layout/UnitTitle");
            var selectedAction = screen.GetNode<Label>("Margin/Layout/BattleBoard/SelectedUnitPanel/Layout/UnitAction");
            ...
            screen.StartBattle(content, MerchantConfig(hero, enemy, rule, summon, 4), "商人余额不足测试");
            command.EmitSignal(BaseButton.SignalName.Pressed);
            if (screen.CurrentMana != 3 || screen.RemainingGold != 4 || screen.TemporaryUnitCount != 0 ||
                !screen.CommandFeedback.Contains("金币不足", StringComparison.Ordinal) || !status.Text.Contains("金币不足", StringComparison.Ordinal))
                throw new InvalidOperationException("merchant insufficient-gold feedback or invariants failed");
            await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
            if (!status.Text.Contains("金币不足", StringComparison.Ordinal))
                throw new InvalidOperationException("merchant insufficient-gold feedback was overwritten by process HUD refresh");

            screen.StartBattle(content, MerchantConfig(hero, enemy, rule, summon, 20), "商人法力耗尽测试");
            command.EmitSignal(BaseButton.SignalName.Pressed);
            command.EmitSignal(BaseButton.SignalName.Pressed);
            command.EmitSignal(BaseButton.SignalName.Pressed);
            if (screen.CurrentMana != 0 || command.Disabled)
                throw new InvalidOperationException("zero-mana command became unclickable before it could report failure");
            var goldAtZero = screen.RemainingGold;
            var summonsAtZero = screen.TemporaryUnitCount;
            command.EmitSignal(BaseButton.SignalName.Pressed);
            if (screen.CurrentMana != 0 || screen.RemainingGold != goldAtZero || screen.TemporaryUnitCount != summonsAtZero ||
                !screen.CommandFeedback.Contains("法力不足", StringComparison.Ordinal) || !status.Text.Contains("法力不足", StringComparison.Ordinal))
                throw new InvalidOperationException("zero-mana attempt swallowed feedback or changed command resources");
        }
        finally
        {
            merchantRoot.Free();
    ...
    private static BattleConfig MerchantConfig(UnitSnapshot hero, UnitSnapshot enemy, HeroRuleSnapshot rule, UnitSnapshot summon, int gold) => new()
    {
        Seed = 91,
