...
public partial class VfxBallisticTrack : Node2D, IVfxTrack
{
    ...
    public override void _Ready()
    {
        ...
        for (int i = 0; i < Mathf.Clamp(Amount, 1, 16); i++)
        {
            var sprite = ParticleScene.Instantiate<Sprite2D>();
            AddChild(sprite);
            sprite.Visible = false;
            var angle = Radial ? Direction + Mathf.Tau * (i + rng.RandfRange(.1f, .9f)) / Mathf.Clamp(Amount, 1, 16)
                : Direction + rng.RandfRange(-Spread, Spread);
            var velocity = Vector2.FromAngle(angle)
            ...
            _fragments.Add(new(sprite, origin,
                velocity, Delay + rng.RandfRange(0, .025f),
    ...
    public void Sample(float age, bool sustained, float release, float impact, bool reducedMotion)
    {
        foreach (var fragment in _fragments)
        {
            ...
            sprite.Rotation = FaceVelocity ? fragment.Velocity.Angle() : 0;
